...
#HC first index is the one we are doing the susceptibility for (i), j is the previous infection
#HC this function can now take matrices for delta, the cross protect lag and lambda
get.cross.protect.data <- function(dat, case.cols,
                                   delta.cpss, k.cpss, lambda.cpss,
                                   birth.lag,
                                   analysis.start.time,
                                   n.iter=10, tol=1e-5,
  ...
  for ( i in 1:4){
    for (j in 1:4){
      if (i!=j){
  if(idx.start.date-k.cpss[i,j]-birth.lag<1)
  ...
  #HC not sure what needs to be in the next line- is is the max of the ks?
  ...
  Q.terms <- array(0, dim=c(n.strains, n.strains, length(analysis.idx)))
  Q.sums <- array(0, dim=c(n.strains, n.strains, length(analysis.idx)))
  Q.sums.total <- array(0, dim=c(n.strains, length(analysis.idx)))
  ...
  while(eps>tol){
    ...
    for ( i in 1:n.strains){
      ...
      fits[[i]] <- reconstruct.susc.class(cum.cases=
                                          ...
                                          cum.births=
                                            cumbirths.lagged,
                                          deltacpss=deltacpss, kcpss=k.cpss,
                                          offset=Q.sums.total[i,],
                                          smooth=smooth,
                                          n.smooth=n.smooth,
                                          df.low=df.low,
                                          df.high=df.high,
    ...
    for ( i in 1:n.strains){
      for ( j in 1:n.strains){
        
        if ( j!=i){
       
        case.idx<- (idx.start.date-kcpss[i,j]):nrow(dat)
          
    if(is.null(lambda.cpss)){
      #HC calculate those returning to the susceptible population for each previous infecting serotype (j) and for susceptibility to each serotype (i)
      #HC the function takes the case counts of the previous infecting serotype (j)
      #HC  for the fixed duration model:
      #HC the function takes the relevant k for that serotype combination (ij)
      
        Q.terms[i,j,] <- calc.offset(case.counts=
                                     dat[case.idx,case.cols[j]],
                                   rho.t=curr.rho[,i],
                                   k=kcpss[i,j])
      ...
      #HC calculate those returning to the susceptible population for each previous infecting serotype (j) and for susceptibility to each serotype (i)
      #HC the function takes the case counts of the previous infecting serotype (j)
      #HC for the exponential model:
      #HC the function takes the relevant k lambda for that serotype combination (i here)  (depending on i and j) ( and the relevant k- does not alter)
    
        Q.terms[i,j,] <- calc.exp.offset(case.counts=
                                         dat[case.idx,case.cols[j]],
                                       rho.t=curr.rho[,i],
                                       k=kcpss[i,j],
                                       lambda=lambdacpss[i,j])
      }    
    
    
        
        
      #HC times each Q term by the relevant delta to get the sums term for each ij combination where i is not equal to j
               Q.sums[i,j,] <- Q.terms[i,j,]*deltacpss[i,j]
        
   
        }
      }
      ## calculate the offsets as the sum of all Q terms for j!=i  
      #HC the total Q term for each serotype i for which we are considering susceptibility for
      Q.sums.total[i,]<-apply(Q.sums[i,-i,],
                            MAR=2,
                            FUN=sum)
    }
    ...
    if(iter==n.iter) {
      print(paste("deltacpss = ", round(deltacpss,3),
                  "kcpss = ", kcpss,
                  "lambdacpss = ", lambdacpss,
...
reconstruct.susc.class <- function(cum.cases, cum.births, deltacpss, kcpss,
                                   offset,
                                   smooth=TRUE, n.smooth=10,
                                   df.low=5, df.high=50,
                                   plot=TRUE){
...
calc.exp.offset <- function(case.counts, rho.t, k, lambda){
  ...
  #HC changed from i to m
  for(m in 1:nobs){
    idx <- (m+k):(m+1) ## index on case.idx scale i=1 => time k+1
    seq2.scaled.cases[m] <- sum(exp.probs*scaled.cases[idx])
  }
...
#HC I have not yet altered the following to deal with the best way to present the max likelihoods of the greater number of parametersHC
#HC comment out all below until the end of the code
#HC TSIR.post.estimation <- function(ll.store, delta1.range, delta2.range, k1.range, k2.range,
#HC                                 DF, model.frmla, ll.col, cor.method="pearson",
#HC                              dat, st.date, plot.cor=FALSE, smooth=TRUE,
#HC                              delta.plot.range=c(-1,1), ...){
#HC   require(lattice)
  
  ## calculate MLEs
#HC liks <- ll.store[,ll.col]
#HC ll.tmp <- cbind(ll.store, liks=liks)
  
#HC ## calculate MLEs
#   idx.to.keep <- (ll.tmp[,"delta1.vec"]>=delta1.range[1] &
#                     ll.tmp[,"delta1.vec"]<=delta1.range[2] &
#                     ll.tmp[,"delta2.vec"]>=delta2.range[1] &
#                     ll.tmp[,"delta2.vec"]<=delta2.range[2] &
#                     ll.tmp[,"k1.vec"]>=k1.range[1] &
#                     ll.tmp[,"k1.vec"]<=k1.range[2]&
#                   ll.tmp[,"k2.vec"]>=k2.range[1] &
#                     ll.tmp[,"k2.vec"]<=k2.range[2])
#   max.lik <- max(liks[idx.to.keep], na.rm=TRUE)
#   mle.idx <- which(liks==max.lik)
#   delta1.mle <- ll.tmp[mle.idx,"delta1.vec"]
#   delta2.mle <- ll.tmp[mle.idx,"delta2.vec"]
#   k1.mle <- ll.tmp[mle.idx,"k1.vec"]
#   k2.mle <- ll.tmp[mle.idx,"k2.vec"]
#   N.fit <- ll.tmp[mle.idx, "N.fit"]
#   
#   ## calculate univariate CIs
#   ci.limit <- max.lik - qchisq(.95, 2)/2
#   ci.idx <- which(liks>ci.limit & idx.to.keep)
#   delta1.limits <- range(ll.tmp[ci.idx, "delta1.vec"])
#   delta2.limits <- range(ll.tmp[ci.idx, "delta2.vec"])
#   k1.limits <- range(ll.tmp[ci.idx, "k1.vec"])
#   k2.limits <- range(ll.tmp[ci.idx, "k2.vec"])
#   
#   delta.cpss.mle<-array(dim=c(n.strains,n.strains))
#   k.cpss.mle<-array(dim=c(n.strains,n.strains))
#   
#  
#   delta.cpss.mle[1:3,1:3]<-delta1.mle
#   delta.cpss.mle[4,1:3]<-delta1.mle
#   delta.cpss.mle[1:3,4]<-delta2.mle
#   delta.cpss.mle[1,1]<-0
#   delta.cpss.mle[2,2]<-0
#   delta.cpss.mle[3,3]<-0
#   delta.cpss.mle[4,4]<-0
#   
#   
#   k.cpss.mle[1:3,1:3]<-k1.mle
#   k.cpss.mle[4,1:3]<-k2.mle
#   k.cpss.mle[1:3,4]<-k2.mle
#   k.cpss.mle[1,1]<-0
#   k.cpss.mle[2,2]<-0
#   k.cpss.mle[3,3]<-0
#   k.cpss.mle[4,4]<-0
...
#                                          delta.cpss=delta.cpss.mle,
#                                          cr.protect.lag.cpss=k.cpss.mle,
...
#   lambdas1 <- ll.tmp[,"delta1.vec"] * ll.tmp[,"k1.vec"]
#   lambda1.ci <- range(lambdas1[in.ci])
#   lambdas2 <- ll.tmp[,"delta2.vec"] * ll.tmp[,"k2.vec"]
#   lambda2.ci <- range(lambdas2[in.ci])
#   lambda1.mle <- delta1.mle*k1.mle
#   lambda2.mle <- delta2.mle*k2.mle
#   
#   
#   ## plots
#   ##HC take these out for now as are in more dimensions...
# #HCif(plot.cor) par(mfrow=c(1,2))
#   
#   #plot.lik.surface(ll.store, ll.col=ll.col, delta.range=delta.range, k.range=k.range, ...)
#   ## get delta plot range
#   #HC plotting.subset <- which(ll.store[,"delta1.vec"] >= delta.plot.range[1] &
#   #HC            ll.store[,"delta1.vec"] <= delta.plot.range[2]&
#   #HC           ll.store[,"delta2.vec"] >= delta.plot.range[1] &
#   #HC             ll.store[,"delta2.vec"] <= delta.plot.range[2])
#   #HCggplot.lik.surface(ll.store[plotting.subset,], ll.col=ll.col, N.fit=N.fit)
#   
#   ## get correlation between observed and predicted
#   #HCyhat <- exp(predict(bestfit))
#   #HCrho.cols <- paste("rho.str", 1:4, sep="")
#   #HCdat.cols <- paste("den.cases.str", 1:4, sep="")
#   #HC#HCrep.factors <- as.matrix(bestfit.data$dat.final[-1, rho.cols])
#   #HCrep.factors <- as.numeric(rep.factors)[bestfit.data$nonzero.subset]
#   #HCyhat.scaled <- yhat/rep.factors
#   #HCobs.y <- as.matrix(bestfit.data$dat.final[-1,dat.cols])
#   #HCobs.y <- as.numeric(obs.y)[bestfit.data$nonzero.subset]
#   
#   #HCspCorr <- cor(obs.y, yhat.scaled, method=cor.method)
#   #HCif(plot.cor) {
#   #HC  plot(jitter(obs.y,2), yhat.scaled, pch=19, cex=.5,
#   #HC    ylim=range(obs.y, yhat.scaled), xlim=range(obs.y, yhat.scaled),
#   #HC    xlab="observed case count (jittered)", ylab="predicted case count")
#   #HCabline(a=0, b=1, col="gray")
#   #HC  par(mfrow=c(1,1))
#   #HC}
#   
#   
...
#               delta1.mle=delta1.mle,
#               delta2.mle=delta2.mle,
#               delta1.ci.95=delta1.limits,
#               delta2.ci.95=delta2.limits,
#               k1.mle=k1.mle/26,
#               k2.mle=k2.mle/26,
#               k1.ci.95=k1.limits/26,
#               k2.ci.95=k2.limits/26,
#               lambda1.mle=lambda1.mle/26,
#               lambda1.ci.95=lambda1.ci/26,
#               lambda2.mle=lambda2.mle/26,
#               lambda2.ci.95=lambda2.ci/26,
...
#   
#   ll.tmp <- data.frame(delta1s = ll.store[,"delta1.vec"],delta2s = ll.store[,"delta2.vec"],
#                        k1s = ll.store[,"k1.vec"],k2s = ll.store[,"k2.vec"],
...
#   delta1.mle <- ll.tmp[mle.idx,"delta1s"]
#   k1.mle <- ll.tmp[mle.idx,"k1s"]
#   delta2.mle <- ll.tmp[mle.idx,"delta2s"]
#   k2.mle <- ll.tmp[mle.idx,"k2s"]
#   
...
#   k1.lim <- max(ll.tmp[,"k1s"])
#   k2.lim <- max(ll.tmp[,"k2s"])
...
#                              delta1.range,delta2.range, k1.range,k2.range, ...) {
#   
#   ll.tmp <- data.frame(delta1s = ll.store[,"delta1.vec"],
#                        delta2s = ll.store[,"delta1.vec"],
#                        k1s = ll.store[,"k1.vec"]/26,
#                        k2s = ll.store[,"k2.vec"]/26,
#                        logliks = ll.store[,ll.col])
#   
#   ## calculate MLEs
#   k1.range <- k1.range/26
#   k2.range <- k2.range/26
#   idx.to.keep <- which(ll.tmp[,"delta1s"]>=delta1.range[1] &
#                          ll.tmp[,"delta1s"]<=delta1.range[2] &
#                          ll.tmp[,"delta2s"]>=delta2.range[1] &
#                          ll.tmp[,"delta2s"]<=delta2.range[2] &
#                          ll.tmp[,"k1s"]>=k1.range[1] &
#                          ll.tmp[,"k1s"]<=k1.range[2]&
#                          ll.tmp[,"k2s"]>=k2.range[1] &
#                          ll.tmp[,"k2s"]<=k2.range[2])
#   max.lik <- max(ll.tmp[idx.to.keep, "logliks"], na.rm=TRUE)
#   mle.idx <- which(ll.tmp[,"logliks"]==max.lik)
#   delta1.mle <- ll.tmp[mle.idx,"delta1s"]
#   k1.mle <- ll.tmp[mle.idx,"k1s"]
...
#   #HC fix delta to 1
#   delta.mle <- 1
#   lambda1.mle <- ll.tmp[mle.idx,"lambda1"]
#   lambda2.mle <- ll.tmp[mle.idx,"lambda2"]
#   delta.cpss.mle<-array(dim=c(n.strains,n.strains))
#   lambda.cpss.mle<-array(dim=c(n.strains,n.strains))
#   delta.cpss.mle[,]<-1
#   lambda.cpss.mle[1:3,1:3]<-lambda1.mle 
#   lambda.cpss.mle[1,1]<-0
#   lambda.cpss.mle[2,2]<-0
#   lambda.cpss.mle[3,3]<-0
#   lambda.cpss.mle[1:3,4]<-lambda2.mle
#   lambda.cpss.mle[4,1:3]<-lambda2.mle
#   
#   N.fit <- ll.tmp[mle.idx, "N.fit"]
#   
#   ## get univariate confidence intervals
#   #HCif(univariate.ci){
#     #HC ci.95.limit <- max.lik - qchisq(.95, 1)/2
#     #HC ci.99.limit <- max.lik - qchisq(.99, 1)/2
#     #HCci.95.idx <- which(liks>ci.95.limit & ll.store[,"delta"]==1)
#     #HC ci.99.idx <- which(liks>ci.99.limit & ll.store[,"delta"]==1)
#   #HC} else {
...
#  #HC }
#   lambda1.ci.95 <- range(ll.tmp[ci.95.idx, "lambda1"])
#   lambda1.ci.99 <- range(ll.tmp[ci.99.idx, "lambda1"])
#   lambda2.ci.95 <- range(ll.tmp[ci.95.idx, "lambda2"])
#   lambda2.ci.99 <- range(ll.tmp[ci.99.idx, "lambda2"])
#   return(list(bestfit=bestfit,
#               bestfit.data=bestfit.data,
#               lambda1.mle=lambda1.mle/26,
#               lambda1.ci.95=lambda1.ci.95/26,
#               lambda2.mle=lambda2.mle/26,
#               lambda2.ci.95=lambda2.ci.95/26,
#               delta.mle=delta.mle,
...
#     theme_bw() + geom_tile() + scale_fill_manual(values=pal, name="log-lik")+
#     geom_point(x=lambda1.mle, y=delta.mle, show_guide=FALSE, shape=3) +
...
#   return(rep.rates)
# }
#HC comment out all from line 432
