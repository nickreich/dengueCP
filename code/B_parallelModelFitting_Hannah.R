...
setwd("codeStrainSpecific") 
source("TSIR_Utils_Hannah_attempt2.R")
##source("TSIR_Utils.R")
n.strains<-4
...
kcpss[] <- max(round(qexp(.75, 1/max(lambdas))))
...
lambdas1.vec <- rep(lambdas, times=(length(lambdas)))
lambdas2.vec <- rep(lambdas, each=(length(lambdas)))
paramsexp <- cbind( lambdas1.vec, lambdas2.vec)
exp.logliks<-array(dim=c(length(paramsexp[,1]), 7))

deltacpss<-array(dim=c(n.strains,n.strains))
...
for ( p in 1:nrow(paramsexp)){
  ...
  lambdacpss[1:3,1:3]<-paramsexp[p,"lambdas1.vec"]
  lambdacpss[4,1:3]<-paramsexp[p,"lambdas2.vec"]
  lambdacpss[1:3,4]<-paramsexp[p,"lambdas2.vec"]
  lambdacpss[1,1]<-0
  lambdacpss[2,2]<-0
  lambdacpss[3,3]<-0
  lambdacpss[4,4]<-0
	...
	tmp <- get.cross.protect.data(bkk.dengue.all.cases,
				      ...
				      delta.cpss=1, cr.protect.lag.cpss=kcpss,
...
exp.logliks[p,] <- c(signif(paramsexp[p,],3), signif(ll.Ea,3), signif(ll.Eb,3), signif(ll.Ec,3), signif(ll.Ed,3), signif(length(tmp$nonzero.subset),3))
  
}
Sys.time()

colnames(exp.logliks) <- c("lambda1", "lambda2", "loglik.Ea", "loglik.Eb", "loglik.Ec", "loglik.Ed", "N.fit")

exp.logliks[which(exp.logliks[,3]==max(exp.logliks[,3])),1:2]
exp.logliks[which(exp.logliks[,4]==max(exp.logliks[,4])),1:2]
exp.logliks[which(exp.logliks[,5]==max(exp.logliks[,5])),1:2]
exp.logliks[which(exp.logliks[,6]==max(exp.logliks[,6])),1:2]
